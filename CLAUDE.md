...
- Token tracking: `_track_tokens(model, inp, out)` + словарь `token_usage: dict[str, dict]`.
- **Цены ($/MTok, in/out) — официальный прайс Anthropic**, прокси даёт скидку сверху:
  `claude-haiku-4-5-20251001` $1/$5 · `claude-sonnet-5` $3/$15 · `claude-opus-5` $5/$25 · `claude-fable-5` $10/$50.
  Прежние значения в коде (Haiku $0.80/$4, Opus $15/$75) — устаревшие, `/cost` из-за них врёт.
  Цены поколения 4.x (`claude-sonnet-4-6`, `claude-opus-4-8`) не проверены — при необходимости
  сверять по https://platform.claude.com/docs/en/about-claude/pricing
- **Окна контекста различаются**: Haiku 4.5 — 200k, всё пятое поколение — 1M. Любые лимиты
  памяти/истории калибруются под МИНИМАЛЬНОЕ окно (200k), иначе `/haiku` уронит чат,
  который нормально работал на Opus. См. «Потолок на чтении».
...
Прямой `client.messages.create(` допустим ТОЛЬКО внутри синхронных функций, которые сами
зовутся из потока. Добавляешь новый вызов — выбери строку из таблицы, не пиши напрямую.
...
## Версионность
Текущая версия: v0.8.2. Теги ставит Алексей вручную: `git tag -a vX.Y.Z`. Не создавай теги самостоятельно.
...
- **Обновить модели**: линейка сдвинулась на поколение — актуальны `claude-sonnet-5`,
  `claude-opus-5`, `claude-fable-5` (Haiku остаётся `claude-haiku-4-5-20251001`).
  В коде зашиты `claude-sonnet-4-6` / `claude-opus-4-8`. Добавить `/fable`, вынести
  модели в реестр `MODELS` вместо строк по месту, поправить цены в token tracking.
  ПЕРЕД правкой проверить, какие строки принимает прокси (`api.apitoken.sale`) —
  он может не знать пятое поколение. Записи в `chat_models` со старыми ID мигрировать
  только после подтверждения.
- **Проверить покрытие `/cost`**: убедиться, что `_track_tokens` вызывается и во
  вспомогательных путях (should_search, translate, капча, extract_*), а не только
  в основном диалоге. Иначе цифра занижена.
...
- **`chat_models` постоянна**: `/haiku` `/sonnet` `/opus` пишут выбор навсегда, отката
  по таймеру нет. С дорогими моделями это риск «переключил и забыл» — решить, нужен ли
  автооткат или достаточно показывать текущую модель в `/help`.
