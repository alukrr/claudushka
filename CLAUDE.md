...
- **402 «insufficient balance» → голый `APIStatusError`** (своего класса в SDK нет),
  ловится `api_errors.is_insufficient_balance()` по `status_code`. НЕ ретраебельно:
  кончились деньги на прокси, чинится только пополнением. Пользователю честное
  «закончились деньги, Алексей в курсе», в лог — «ПОПОЛНИ БАЛАНС». До v0.9.2 падало
  в общую ветку, и пользователь видел «сломалось на моей стороне» (41 раз 2026-08-03).
