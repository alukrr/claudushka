...
- **400 с `prompt is too long` → `BadRequestError`**, ловится `api_errors.is_prompt_too_long()`
  по подстроке. Не ретраебельно, лечится обрезкой истории.
...
1. **SDK-ретраи** (`ANTHROPIC_SDK_RETRIES = 3` в конструкторе `anthropic.Anthropic`) —
   для СИНХРОННЫХ вспомогательных вызовов (should_search, перевод промпта, капча,
   extract_memory, greet, daily_chat_review). SDK ретраит 408/409/429/5xx, бэкофф 0.5→8с.
2. **Свои ретраи** (`api_errors.call_with_retry`, паузы `RETRY_DELAYS = (2, 6, 15)`) —
   для пользовательских вызовов через `bot.call_claude()`. Работают на `client_noretry`
   (`client.with_options(max_retries=0)`), иначе задержки перемножились бы.
   Между попытками шлётся `send_chat_action('typing')`, чтобы не выглядело зависанием.

`call_claude()` гоняет `messages.create` через `asyncio.to_thread` — блокирующий вызов
больше не держит event loop телеграм-бота. Новые пользовательские вызовы API добавлять
ТОЛЬКО через `call_claude()`, не напрямую.

### 400 prompt is too long: не заклинивать чат
...
навсегда (так было в инциденте 2026-07-26). Лечение: при `is_prompt_too_long` история
режется вдвое (`_halve_history`, первым обязан идти user — иначе API вернёт другой 400)
и запрос повторяется ОДИН раз. Если снова — пользователю «контекст переполнен, сделай
/clear», в лог `PROMPT TOO LONG` с размерами компонентов. В WhatsApp команд нет, поэтому
подсказка про сброс задаётся параметром `clear_hint`.

### Логирование размера промпта
Перед каждым основным вызовом пишется строка
`PROMPT uid=... chat=... model=... system=... history_chars=... msgs=...`
(для фото — `kind=photo` + размер base64). Именно этих цифр не хватало при разборе
июльского инцидента — не удалять.
...
## Версионность
Текущая версия: v0.8.1. Теги ставит Алексей вручную: `git tag -a vX.Y.Z`. Не создавай теги самостоятельно.
