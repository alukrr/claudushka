...
- Контекст для модели строит `build_group_messages()` (bot.py) из `db.get_group_transcript(chat_id, MAX_HISTORY)`: реплики людей → user-блоки с подписью «Имя: текст» (подряд идущие склеиваются), ответы Клодушки → assistant-блоки. Гарантируется чередование ролей и user первым.
...
- Память в группах: `extract_memory` всё ещё пишет факты по `user_id` автора, а не на уровень чата. Для многоголосого чата это неточно — запланирован переход на память уровня чата (об участниках и их отношениях). Это известное ограничение, не баг; правится отдельной подзадачей.
