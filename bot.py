...
def get_system_prompt(user_id: int, is_group: bool = False, chat_id: int = None,
    ...
    base = (
        ...
        "В истории чата ты можешь увидеть свои прошлые сообщения вида «(в чат отправлена картинка по промпту: ...)» — это служебная пометка о том, что картинка УЖЕ была отправлена раньше, а не образец, который надо копировать. "
...
# преимущественно латиницу в захваченном хвосте — иначе словим и легитимные русские
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # русские фразы вроде «не буду рисовать картинку: это тупо».
        if draw_en_prompt is None:
            leak_match = LEAKED_DRAW_RE.search(assistant_text)
            if leak_match:
