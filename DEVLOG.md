...
**GPT Image 2.5 Flare + Sunburst как провайдеры картинок платного режима** (ветка `feat/gpt-image-25`)

- Два новых провайдера картинок через тот же пул `api.apitoken.sale`, что и GPT Image 2 —
  `openai/gpt-image-2.5-flare` и `openai/gpt-image-2.5-sunburst` (ID с префиксом `openai/`,
  в отличие от голого `gpt-image-2`). Команды `/flare` (алиас `/gpt25f`) и `/sunburst`
  (алиас `/gpt25s`), доступны только в платном режиме (как `/banana`). Цена — $0.02, как у
  GPT Image 2.
- `IMAGE_PROVIDERS` пополнился полями `cmds`/`backend`/`model_id`; диспетчеризация
  генерации теперь идёт по `backend` ("pool"/"gemini"), а не по хардкоду ключа "gpt" —
  раньше новый провайдер через пул молча ушёл бы в banana. `_try_gpt_image` параметризован
  `model_id`/`label`, обслуживает все три "pool"-провайдера. Регистрация команд —
  циклом по реестру, с проверкой на дубли имён при старте.
- `get_chat_image_provider` теперь устойчив к незнакомому ключу в БД (откат ветки и т.п.) —
  фоллбек на дефолт с warning в лог, а не `KeyError`.
- Подробности — `docs/claude/images.md`, `docs/claude/billing.md`.
