...
- **`requests` не зафиксирован в `requirements.txt`, ждём версию от Алексея (2026-09-19).**
  `bot.py` импортирует `requests` напрямую (`import requests as
  http_requests`), но в `requirements.txt` его нет — приезжает транзитивно (вероятно,
  через `tavily-python` или `gradio_client`, последний убран этой же правкой; без него
  транзитивная цепочка могла измениться). Незафиксированная транзитивная версия может
  тихо смениться при пересборке образа. Версию нужно взять РЕАЛЬНУЮ из контейнера, не
  угадывать:
  ```bash
  docker exec claudushka pip show requests
  ```
  После ответа — добавить `requests==X.Y.Z` в `requirements.txt` (рядом с `httpx`).

- **Fable 5 → Fable 5.1 (`claude-fable-5-1`), ждём пробный запрос Алексея (2026-09-19).**
  Официальный прайс Anthropic уже перечисляет Fable 5.1 (та же цена
  ...
  ```
  `claude-sonnet-5` в этой же команде — сверка для `WA_MODEL` в whatsapp.py: bot.py им
  уже пользуется в проде для `/sonnet`, ожидаемо 200, код уже переведён
