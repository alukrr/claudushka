...
- **Гигиена памяти**: дедуп фактов (`UNIQUE(user_id, context, chat_id, fact)` +
  ...
  **У этого ключа/пула включено автоматическое prompt caching** — не только для
  ...
  `cache_creation_input_tokens=16186`). Множители кэша (в `PRICING` скрипта — per-model, запись 5m/1h
  раздельно, синхронно с bot.py) это учитывают. Стоит держать в уме и за пределами этого скрипта — если где-то ещё в
...
## После ТЗ v0.10 (v1.0.0) — отложено
Стенд по `docs/claude/staging-checklist-v0.10.md` пройден 2026-09-20 (B1–B14, включая миграцию на копии прод-БД
и обзор на Opus), выкачено на прод. Остаётся:
- Free-группы за счёт владельца расходуют Haiku на `extract_all_participants_memory` (раз в 15 реплик,
  `billed=0`) — не ограничено; если станет заметным в `/cost` («бесплатный режим»), гейтить по проверке/тарифу.
- Не учитывается в `usage_log`: Gemini-транскрипция аудио (`_transcribe_audio_gemini`). ТЗ знает только
  `llm|image|search`.
- `handle_captcha` по-прежнему мёртвый код (нигде не вызывается), `/captcha_on` фактически влияет только на `/start`.
- Retention `usage_log` не делали (ТЗ: «сейчас не делать») — таблица растёт по строке на вызов.
- Цена GPT-картинки `IMAGE_PRICES["gpt"]=0.02` — осознанная наценка ×2 к себестоимости пула ($0.01, решение
  Алексея 2026-09-20). Если пул поменяет скидку — цену перепроверять по счёту.
- Файл без подписи в группе виден боту только как плейсхолдер; реплай на документ содержимое не подтягивает.
  Решено не делать (2026-09-20).

## После Opus 5.5 (2026-09-23) — отложено
- **`effort` для коротких ответов**: `/review` и дневной обзор на Opus 5.5 думают на дефолтном
  `medium`, thinking оплачивается как output ($20/MTok). `output_config={"effort": "low"}` для
  не-Haiku моделей (у Haiku 4.5 параметр — ошибка) удешевит их, но может сказаться на качестве
  текста — не делали без решения Алексея. Данные для решения — `usage_log.thinking`.
