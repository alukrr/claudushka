...
## После ТЗ v0.10 (учёт/тарифы) — отложено и непроверенное
- **Стенд не пройден** (ТЗ требует): контекст п.2 (строки `usage_log` с `chat_id` у `should_search`/`extract_*`/
  перевода промпта), `/topup` и авто-проверка от $5, уход в ноль с writeoff и сообщением, лимиты картинок free,
  лимиты непроверенных в личке и группе (в т.ч. проверенный юзер в непроверенной группе), возврат в paid с
  прежним выбором, двойной `/fable`, `/cost` на трёх уровнях, обзор на Opus, ban/unban с сохранением баланса,
  новая группа без бонуса. Чек-лист — `docs/claude/staging-checklist.md`, раздел «ТЗ v0.10».
- Проверено локально (не на стенде): миграции на копии старой схемы, баланс/writeoff/переход paid→free
  (`db.record_usage`), `usage_ctx` через настоящий `Application.process_update` → `to_thread`, гейт банов,
  лимиты, `/fable`, `/topup`, `/cost` — скриптами с временной БД и заглушками Telegram.
- Free-группы бесплатно для владельца расходуют Haiku на `extract_all_participants_memory` (раз в 15 реплик,
