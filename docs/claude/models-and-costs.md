...
| `fable` | `claude-fable-5-1` | Fable 5.1 | 10 / 50 | 0.025× | 1M | только админ |
...
CACHE_READ_MULTIPLIER)` из реестра `MODELS` (2026-09-19) — 0.1x у Haiku/Sonnet/Opus,
0.025x у Fable 5.1 (`CACHE_READ_MULTIPLIER`=0.1 остаётся только дефолтом для строк ВНЕ
реестра — легаси в `chat_models`, до применения идемпотентной миграции). `token_usage` в памяти, сбрасывается при рестарте — старые
