...
**Подпись к картинке — только `_photo_caption()`** (лимит Telegram 1024 в UTF-16, режется
промпт, не хвост; при `BadRequest` — повтор с короткой подписью). Голая f-строка теряла
оплаченные картинки на длинных промптах Opus 5.5 — инцидент 2026-09-23 в `incidents.md`.
